...
<html>
<head>
  <title>Data Solusi - <?=$konfig['judul']?></title>
      ...
      <div class="card">
        ...
        <div class="card-body no-padding">
          <table class="datatable table table-striped primary" cellspacing="0" width="100%">
            ...
            <tbody>
                <?php
                  $sql = mysql_query("SELECT * FROM solusi");
                  $no = 1;
                  while ($row = mysql_fetch_array($sql)) {
                    echo '<tr>
                                <td>'.$no.'</td>
                                <td>'.$row['solusi'].'</td>
                                <td><button type="button" data-toggle="modal" data-target="#myModal'.$row['id_solusi'].'" class="btn btn-primary">Update</button> <button type="button" class="btn btn-warning" onclick=\'deletedata("'.$row['id_solusi'].'")\'>Delete</button>
                                <div class="modal fade" id="myModal'.$row['id_solusi'].'" role="dialog">
                                      ...
                                      <div class="modal-body">
                                        ...
                                        <input type="hidden" name="id_solusi" value="'.$row['id_solusi'].'">
                                            <div class="input-group">
                                              <input type="text" class="form-control" name="upsolusi" id="upsolusi" placeholder="Solusi" value="'.$row['solusi'].'" required>
                                            </div>
                                         </div>
  ...
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Solusi Baru</h4>
        </div>
        <div class="modal-body">
          <form id="forminput">
              <div class="input-group">
                <input type="text" class="form-control" id="solusi" placeholder="Solusi" required>
              </div>
           </form>
        </div>
        <div class="modal-footer">
...
<?php
  if (isset($_POST['update'])) {
    ...
    $upsolusi = $_POST['upsolusi'];
    $sql = mysql_query("UPDATE solusi SET solusi='$upsolusi' WHERE id_solusi='$id_solusi'");
    if ($sql) {
    echo '<script>swal({
  title: "Berhasil!",
  type: "success",
...
setTimeout(function(){
        window.location = "'.$konfig["url"].'admin/solusi.php";
        }, 2000);
  </script>';
  } else {
     echo '<script>swal({
...
setTimeout(function(){
  ...
  <script>
    $('#save').click(function(){
      ...
      var datas="solusi="+solusi;

      $.ajax({
         type: "POST",
         url: "./ajax/solusi/save.php",
         data: datas
      }).done(function( data ) {
        $('#myModal').modal('hide')
...
<script>
  ...
  showLoaderOnConfirm: true,
      ...
      $.ajax({
         type: "POST",
         url: "./ajax/solusi/delete.php?id="+id
      }).done(function( data ) {
