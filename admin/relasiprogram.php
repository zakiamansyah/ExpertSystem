...
<html>
<head>
  <title>Data Relasi Details Program - <?=$konfig['url']?></title>
...
<div class="row">
  <div class="col-xs-12">
      <div class="card">
        <div class="card-body">
          <form method="post" action="./ajax/relasi/addrelasiprogram.php" target="_self" id="forminput">
	 <div class="form-group"> 
	   <label class="col-md-3  control-label">Nama Program</label>
	   <div class="col-md-9">
	   <select class="select2" onChange="MM_jumpMenu('parent',this,0)">
	        <option value="<?=$_SERVER['PHP_SELF'];?>"> [ Daftar Program ]</option>
			<?php
			$sqlp = "SELECT * FROM artikel ORDER BY id_artikel";
			$qryp = mysql_query($sqlp) or die ("SQL Error: ".mysql_error());
			while ($datap=mysql_fetch_array($qryp)) {
				if($datap['id_artikel']==$program) {
					$cek ="selected";
				}else{
				$cek ="";
				}

			echo "<option value='?program=$datap[id_artikel]' $cek> $datap[judul] </option>";
      ...
      <input name="program" type="hidden" id="program" value="<?=$program; ?>" /></td>
      <?php if (isset($_GET['program'])) {?>
      <div class="form-group"> 
	   <label class="col-md-3  control-label">Daftar Kerusakan</label>
	   <div class="col-md-9">
	<?php
	$sql = "SELECT * FROM rusak ORDER BY id_rusak";
	$qry = mysql_query($sql) or die("SQL Error:".mysql_error());
	$no=0;
	while ($data = mysql_fetch_array($qry)){
		$no++;
		$sqlr = "SELECT * FROM relasiprogram WHERE id_artikel='$program' AND id_rusak='$data[id_rusak]'";
		...
		?>
	<div class="checkbox">
	  <input name="CekRusak[]" id="chk<?= $no; ?>" type="radio" value="<?= $data['id_rusak'];?>" <?= $cek; ?> > 
	  <label for="chk<?= $no; ?>">
	  <?= $data['nama_rusak'];?> 
	  </label>
	</div>
