...
<html>
<head>
  <title>Data Gejala - <?=$konfig['judul']?></title>
  ...
  <div class="col-xs-12">
      <div class="card">
        <div class="card-header">
          ...
          <a href="./relasigejala.php" class="btn btn-success"><i class="icon fa fa-database fa-1x"></i> Relasi Data Gejala & Solusi </a>
        </div>
                ...
                <?php
                  ...
                  while ($row = mysql_fetch_array($sql)) {
                    echo '<tr>
                                <td>'.$no.'</td>
                                <td>'.$row['gejala'].'</td>
                                <td><button type="button" data-toggle="modal" data-target="#myModal'.$row['id_gejala'].'" class="btn btn-primary">Update</button> <button type="button" class="btn btn-warning" onclick=\'deletedata("'.$row['id_gejala'].'")\'>Delete</button>
                                <div class="modal fade" id="myModal'.$row['id_gejala'].'" role="dialog">
                                      ...
                                      <div class="modal-body">
                                        ...
                                        <input type="hidden" name="id_gejala" value="'.$row['id_gejala'].'">
                                            <div class="input-group">
                                              <input type="text" class="form-control" name="upgejala" id="upgejala" placeholder="Nama Gejala" value="'.$row['gejala'].'" required>
                                            </div>
                                         </div>
  ...
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Gejala Baru</h4>
        </div>
        <div class="modal-body">
          <form id="forminput">
              <div class="input-group">
...
<?php
  if (isset($_POST['update'])) {
    ...
    $upgejala = $_POST['upgejala'];
    $sql = mysql_query("UPDATE gejala SET gejala='$upgejala',bobot='$upbobot' WHERE id_gejala='$id_gejala'");
if ($sql) {
    echo '<script>swal({
  title: "Berhasil!",
  type: "success",
...
setTimeout(function(){
        window.location = "'.$konfig["url"].'admin/gejala.php";
        }, 2000);
  </script>';
  } else {
     echo '<script>swal({
...
<script>
  ...
  showLoaderOnConfirm: true,
      ...
      $.ajax({
         type: "POST",
         dataType: "json",
         url: "./ajax/gejala/delete.php?id="+id
      }).done(function( data ) {
        if (data.status == 'sukses') {
